...
if(isset($_POST['search']))
{
  ...
  $c = " for Rupees ";
  $d = ucfirst(strtolower($rows['price']));
  $e = ". Contact ";
  $f = ucfirst(strtolower($rows['store_contact']));
  $g = ". ";
  $h = ucfirst(strtolower($rows['comment']));
...
$sql = "SELECT * FROM `expert_stock` ORDER BY `expert_stock`.`stock_id` ASC ";
...
{
    $sql = "SELECT * FROM `expert_stock` ORDER BY `expert_stock`.`state` ASC";
}
elseif ($_GET['sort'] == 'date')
{
    $sql = "SELECT * FROM `expert_stock` ORDER BY `expert_stock`.`avail_date` DESC";
}

$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo "<table><tr><th>Stock_id</th><th>Username</th><th>Seeds</th><th>Price(Rs.)</th><th>Avail.Date</th><th>Store_Address</th><th>Store_Contact</th><th>State</th><th>Message [Special]</th></tr>";
...
?>
        ...
        <th><a href="adminStock.php?sort=id" style="text-decoration: none;"><strong>ID</strong></a></th>&nbsp;
        <th><a href="adminStock.php?sort=state" style="text-decoration: none;"><strong>STATE</strong></a></th>&nbsp;
        <th><a href="adminStock.php?sort=date" style="text-decoration: none;"><strong>DATE</strong></a></th>
       </p> </div>
<br><br>
<div class="content">

<form method = "POST">
 <input type = "text" name = "id" placeholder = "Enter Stock_id (TEXT)">
...
document.addEventListener("click", closeAllSelect);
 ...
 <input type = "submit" name = "search" value = "Send Text Message"><br><br>


 
<hr class="solid">
<br>
 <input type = "text" name = "id1" placeholder = "Enter Stock_id (VOICE)">
